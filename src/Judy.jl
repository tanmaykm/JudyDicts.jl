...
show(io::IO, j::JudyArray{Int, Bool}) = println(io, (j.jarr == C_NULL) ? "$(typeof(j)) (empty)" : "$(typeof(j)) (count: $(dec(ju_count(j))))")
show(io::IO, j::JudyArray{Int, Int}) = println(io, (j.jarr == C_NULL) ? "$(typeof(j)) (empty)" : "$(typeof(j)) (count: $(dec(ju_count(j))))")
show{K,V}(io::IO, j::JudyArray{K, V}) = println(io, (j.jarr == C_NULL) ? "$(typeof(j)) (empty)" : "$(typeof(j)) (populated)")
...
ju_free(arr::JudyArray{Int, Bool}) = @_ju_free arr :Judy1FreeArray
ju_free{V}(arr::JudyArray{Int,V}) = @_ju_free arr :JudyLFreeArray
ju_free{V}(arr::JudyArray{String,V}) = @_ju_free arr :JudySLFreeArray
ju_free{V}(arr::JudyArray{Array{Uint8},V}) = @_ju_free arr :JudyHSFreeArray

## returns bytes of memory used currently by the judy array
ju_mem_used(arr::JudyArray{Int, Bool}) = ccall((:Judy1MemUsed, _judylib), Uint, (Ptr{Void},), arr.jarr)
ju_mem_used{V}(arr::JudyArray{Int,V}) = ccall((:JudyLMemUsed, _judylib), Uint, (Ptr{Void},), arr.jarr)
...
setindex!(arr::JudyArray{Int, Bool}, isset::Bool, idx::Int) = isset ? ju_set(arr, idx) : ju_unset(arr, idx)
getindex(arr::JudyArray{Int, Bool}, idx::Int) = (ju_get(arr, idx) == 1)

setindex!(arr::JudyArray{Int, Int}, val::Any, idx::Int) = @_unset arr val idx
setindex!(arr::JudyArray{Int, Int}, val::Int, idx::Int) = (C_NULL != ju_set(arr, idx, val)) ? val : error("Error setting value")
setindex!{V}(arr::JudyArray{Int,V}, val::V, idx::Int) = (C_NULL != ju_set(arr, idx, val)) ? val : error("Error setting value")
getindex(arr::JudyArray{Int, Int}, idx::Int) = @_get arr idx
...
getindex(arr::JudyArray{Array{Uint8}, Int}, idx::Array{Uint8}) = @_get arr idx
...
function ju_set(arr::JudyArray{Int, Bool}, idx::Int)
...
function ju_set{V}(arr::JudyArray{Int,V}, idx::Int, val::V)
...
function ju_set(arr::JudyArray{String, Int}, idx::String, val::Int)
...
function ju_set(arr::JudyArray{Array{Uint8}, Int}, idx::Array{Uint8}, val::Int)
...
ju_get(arr::JudyArray{Int, Bool}, idx::Int) = ccall((:Judy1Test, _judylib), Int32, (Ptr{Void}, Uint, Ptr{Void}), arr.jarr, convert(Uint, idx), C_NULL)
...
function ju_get(arr::JudyArray{Array{Uint8}, Int}, idx::Array{Uint8})
...
ju_get(arr::JudyArray{Int, Int}, idx::Int) = @_ju_get arr convert(Uint, idx) Uint :JudyLGet
ju_get(arr::JudyArray{String, Int}, idx::String) = @_ju_get arr bytestring(idx) Ptr{Uint8} :JudySLGet
function ju_get{V}(arr::JudyArray{Int,V}, idx::Int)
...
ju_unset(arr::JudyArray{Int, Bool}, idx::Int) = @_ju_unset arr convert(Uint, idx) Uint :Judy1Unset
ju_unset(arr::JudyArray{Int, Int}, idx::Int) = @_ju_unset arr convert(Uint, idx) Uint :JudyLDel
function ju_unset{V}(arr::JudyArray{Int,V}, idx::Int)
    ret_tuple = @_ju_get arr convert(Uint, idx) Uint :JudyLGet
    if(ret_tuple[2] != C_NULL)
...
ju_count(arr::JudyArray{Int, Bool}, idx1::Int, idx2::Int) = ccall((:Judy1Count, _judylib), Uint, (Ptr{Void}, Uint, Uint, Ptr{Void}), arr.jarr, convert(Uint, idx1), convert(Uint, idx2), C_NULL)
ju_count(arr::JudyArray{Int, Bool}) = ju_count(arr, 0, -1)
ju_count{V}(arr::JudyArray{Int,V}, idx1::Int, idx2::Int) = ccall((:JudyLCount, _judylib), Uint, (Ptr{Void}, Uint, Uint, Ptr{Void}), arr.jarr, convert(Uint, idx1), convert(Uint, idx2), C_NULL)
ju_count{V}(arr::JudyArray{Int,V}) = ju_count(arr, 0, -1)
...
function ju_by_count(arr::JudyArray{Int, Bool}, n::Int)
...
function ju_by_count(arr::JudyArray{Int,Int}, n::Int)
...
function ju_by_count{V}(arr::JudyArray{Int,V}, n::Int)
    ret::Ptr{Uint} = ccall((:JudyLByCount, _judylib), Ptr{Uint}, (Ptr{Void}, Uint, Ptr{Uint}, Ptr{Void}), arr.jarr, convert(Uint, n), arr.nth_idx, C_NULL)
    ret_val::Uint = (ret != C_NULL) ? unsafe_ref(ret) : C_NULL
...
start(arr::JudyArray{Int, Bool}) = ju_first(arr)
done(arr::JudyArray{Int, Bool}, state) = (0 == state[1])
next(arr::JudyArray{Int, Bool}, state) = done(arr, state) ? (Nothing, state) : ((true, state[2]), ju_next(arr))

start{V}(arr::JudyArray{Int, V}) = ju_first(arr)
...
ju_first(arr::JudyArray{Int, Bool}, idx::Int) = ju_first(arr, convert(Uint, idx))
function ju_first(arr::JudyArray{Int, Bool}, idx::Uint)
    arr.nth_idx[1] = idx
    ret::Int32 = ccall((:Judy1First, _judylib), Int32, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    return (ret, arr.nth_idx[1])
end

ju_first{V}(arr::JudyArray{Int, V}) = ju_first(arr, 0)
function ju_first(arr::JudyArray{Int, Int}, idx::Int)
...
function ju_first{V}(arr::JudyArray{Int, V}, idx::Int)
    arr.nth_idx[1] = convert(Uint, idx)
    ret::Ptr{Uint} = ccall((:JudyLFirst, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    if(ret != C_NULL)
...
ju_next(arr::JudyArray{Int, Bool}, idx::Int) = ju_next(arr, convert(Uint, idx))
function ju_next(arr::JudyArray{Int, Bool}, idx::Uint)
    arr.nth_idx[1] = idx
    ju_next(arr)
end
function ju_next(arr::JudyArray{Int, Bool})
    ret::Int32 = ccall((:Judy1Next, _judylib), Int32, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    return (ret, arr.nth_idx[1])
end


function ju_next{V}(arr::JudyArray{Int, V}, idx::Int)
    arr.nth_idx[1] = convert(Uint, idx)
    ju_next(arr)
end
function ju_next(arr::JudyArray{Int, Int})
    ret::Ptr{Uint} = ccall((:JudyLNext, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    ret_val::Uint = (ret != C_NULL) ? unsafe_ref(ret) : C_NULL
    return (ret_val, ret, arr.nth_idx[1])
end
function ju_next{V}(arr::JudyArray{Int, V})
    ret::Ptr{Uint} = ccall((:JudyLNext, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    if(ret != C_NULL)
        return (arr.gc_store[unsafe_ref(ret)], ret, arr.nth_idx[1])
    end
...
ju_last(arr::JudyArray{Int, Bool}) = ju_last(arr, -1)
ju_last(arr::JudyArray{Int, Bool}, idx::Int) = ju_last(arr, convert(Uint, idx))
function ju_last(arr::JudyArray{Int, Bool}, idx::Uint)
    arr.nth_idx[1] = idx
    ret::Int32 = ccall((:Judy1Last, _judylib), Int32, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    return (ret, arr.nth_idx[1])
end

ju_last{V}(arr::JudyArray{Int, V}) = ju_last(arr, -1)
ju_last{V}(arr::JudyArray{Int, V}, idx::Int) = ju_last(arr, convert(Uint, idx))
function ju_last(arr::JudyArray{Int, Int}, idx::Uint)
    arr.nth_idx[1] = idx
    ret::Ptr{Uint} = ccall((:JudyLLast, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    ret_val::Uint = (ret != C_NULL) ? unsafe_ref(ret) : C_NULL
    return (ret_val, ret, arr.nth_idx[1])
end
function ju_last{V}(arr::JudyArray{Int, V}, idx::Uint)
    arr.nth_idx[1] = idx
    ret::Ptr{Uint} = ccall((:JudyLLast, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    if(ret != C_NULL)
        return (arr.gc_store[unsafe_ref(ret)], ret, arr.nth_idx[1])
...
function ju_last(arr::JudyArray{String, Int})
...
function ju_last(arr::JudyArray{String, Int}, idx::String)
    @assert length(idx) < MAX_STR_IDX_LEN
    @_strcpy arr.nth_idx bytestring(idx)
    ret::Ptr{Uint} = ccall((:JudySLLast, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint8}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    if(ret != C_NULL)
...
function ju_prev(arr::JudyArray{Int, Bool}, idx::Uint)
    arr.nth_idx[1] = idx
    ju_prev(arr)
end
function ju_prev(arr::JudyArray{Int, Bool})
    ret::Int32 = ccall((:Judy1Prev, _judylib), Int32, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    return (ret, arr.nth_idx[1])
end


function ju_prev{V}(arr::JudyArray{Int, V}, idx::Int)
    arr.nth_idx[1] = convert(Uint, idx)
    ju_prev(arr)
end
function ju_prev(arr::JudyArray{Int, Int})
    ret::Ptr{Uint} = ccall((:JudyLPrev, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    ret_val::Uint = (ret != C_NULL) ? unsafe_ref(ret) : C_NULL
    return (ret_val, ret, arr.nth_idx[1])
end
function ju_prev{V}(arr::JudyArray{Int, V})
    ret::Ptr{Uint} = ccall((:JudyLPrev, _judylib), Ptr{Uint}, (Ptr{Void}, Ptr{Uint}, Ptr{Void}), arr.jarr, arr.nth_idx, C_NULL)
    if(ret != C_NULL)
        return (arr.gc_store[unsafe_ref(ret)], ret, arr.nth_idx[1])
    end
...
function ju_prev(arr::JudyArray{String, Int})
...
ju_first_empty(arr::JudyArray{Int, Bool}) = ju_first_empty(arr, 0)
ju_first_empty(arr::JudyArray{Int, Bool}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :Judy1FirstEmpty

ju_first_empty{V}(arr::JudyArray{Int, V}) = ju_first_empty(arr, 0)
ju_first_empty{V}(arr::JudyArray{Int, V}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :JudyLFirstEmpty

ju_next_empty(arr::JudyArray{Int, Bool}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :Judy1NextEmpty
ju_next_empty(arr::JudyArray{Int, Bool}) = @_ju_iter_empty_cont arr :Judy1NextEmpty

ju_next_empty{V}(arr::JudyArray{Int, V}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :JudyLNextEmpty
ju_next_empty{V}(arr::JudyArray{Int, V}) = @_ju_iter_empty_cont arr :JudyLNextEmpty

ju_last_empty(arr::JudyArray{Int, Bool}) = ju_last_empty(arr, -1)
ju_last_empty(arr::JudyArray{Int, Bool}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :Judy1LastEmpty

ju_last_empty{V}(arr::JudyArray{Int, V}) = ju_last_empty(arr, -1)
ju_last_empty{V}(arr::JudyArray{Int, V}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :JudyLLastEmpty

ju_prev_empty(arr::JudyArray{Int, Bool}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :Judy1PrevEmpty
ju_prev_empty(arr::JudyArray{Int, Bool}) = @_ju_iter_empty_cont arr :Judy1PrevEmpty

ju_prev_empty{V}(arr::JudyArray{Int, V}, idx::Int) = @_ju_iter_empty arr convert(Uint, idx) :JudyLPrevEmpty
ju_prev_empty{V}(arr::JudyArray{Int, V}) = @_ju_iter_empty_cont arr :JudyLPrevEmpty
